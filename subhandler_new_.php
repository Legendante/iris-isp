...
$Subdomain = pebkac($_GET['dom'], 100, 'STRING');
...
foreach($PackageArr AS $PackageID => $PackageRec)
{
	if($PackageRec['speedid'] > 0)
	{
		...
		if($PackageRec['ontid'] > 0)
		{
			...
			$ONTInfo[$PackageRec['ontid']] = $PackageRec['ontcost'];
		}
...
// print_r($UnitOrdered);
$NumUnits = getComplexUnitMapCount($ComplexID); //$ComplexRec['numunits'];
...
function loadPackage()
{
	...
	// $.ajax({async: false, type: "POST", url: "ajaxGetPackageDeets.php", dataType: "json",
		...
		// {
			// console.log(feedback.packageid);
...
<div id='comwindow'>
	<div class="form-group">
		<label class="col-sm-3 control-label">Complex:</label>
		<div class="col-sm-4"><h3><?php echo $ComplexRec['complexname']; ?></h3></div>
	</div>
	<div class="form-group">
...
foreach($Speeds AS $SpeedID => $SpeedRec)
{
	...
	if($cnt == 0)
	{
		echo "<td rowspan='" . count($Speeds) . "' valign='top'>";
		foreach($Months AS $TermID => $Termname)
		{
			echo "<input type='radio' name='mntterms' id='mntterms_" . $TermID . "' value='" . $TermID . "' onchange='loadPackage();'>" . $Termname . "<br>";
		}
		// echo "<select name='packterm' id='packterm' onchange='loadPackage();'>";
		// foreach($Months AS $TermID => $Termname)
		// {
			// if($TermID > 0)
		...
		echo "</td>";
		echo "<td rowspan='" . count($Speeds) . "' valign='top'>";
		// echo "<select name='packont' id='packont' onchange='loadPackage();'>";
		foreach($ONTs AS $OntID)
		{
			echo "<input type='radio' name='packont' id='packont_" . $OntID . "' value='" . $OntID . "' onchange='loadPackage();' data-ontcost='" . $ONTInfo[$OntID] . "'>" . $VendorONTs[$OntID] . " " . $ONTInfo[$OntID] . "<br>";
		}
		// echo "<select name='packont' id='packont' onchange='loadPackage();'>";
		// foreach($ONTs AS $OntID)
		// {
...
?>
			<tr><td id='packdetails' colspan='4'>
				<table class='table'>
				<tr><th>Monthly Cost</th><th>ONT Cost <small>(per month)</small></th><th>Connection Fee <small>(Once off)</small></th><th>Once off total</th></tr>
				<tr>
