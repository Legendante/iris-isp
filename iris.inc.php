...
function getCustomerByID($CustomerID)
{
	...
	$selQry .= 'billingid, billingname, billingcontact, billingemail, billingcell, unitnumber, streetaddress1, streetaddress2, streetaddress3, streetaddress4, streetaddress5, customerbalance ';
	...
	while($selData = mysqli_fetch_array($selRes))
	{
		...
		$retArr['streetaddress4'] = $selData['streetaddress4'];
		$retArr['streetaddress5'] = $selData['streetaddress5'];
		$retArr['customerbalance'] = $selData['customerbalance'];
	}
	return $retArr;
}
...
function getCustomersByIDList($CustomerIDList)
{
	...
	while($selData = mysqli_fetch_array($selRes))
	{
		...
		$retArr[$selData['customerid']]['customerbalance'] = $selData['customerbalance'];
	}
	return $retArr;
}
...
function getONTTypes()
{
	...
	$selQry = 'SELECT ontid, ontname, ontcost FROM onttypes WHERE ontid > 0 ORDER BY ontname DESC';
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
		$retArr[$selData['ontid']]['ontname'] = $selData['ontname'];
		$retArr[$selData['ontid']]['ontcost'] = $selData['ontcost'];
	}
	return $retArr;
}
...
function getPackageSpeeds2()
{
	global $dbCon;

	$selQry = 'SELECT speedid, speedname, thespeed FROM packagespeeds ORDER BY speedname * 1';
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
		$retArr[$selData['speedid']]['name'] = $selData['speedname'];
	}
	return $retArr;
}
...
function updateUnitPackage($OrderID, $SaveArr)
{
	global $dbCon;
	...
	$updQry = 'UPDATE unitorderdetails SET ' . $updStr . ' WHERE orderid = ' . $OrderID;
	$updRes = mysqli_query($dbCon, $updQry) or logDBError(mysqli_error($dbCon), $updQry, __FILE__, __FUNCTION__, __LINE__);
}
...
function getFibrePackages($WhereCriteria = '')
{
	...
	$selQry = 'SELECT packageid, packagename, termnum, speedid, ontid, vendorid, ontcost, connectcost, monthlycost, prevatsalesprice, costprice, networkconnectcost, networkontcost, networkinstallcost, prevatontcost, prevatconnectcost, prevatmonthlycost, ';
	$selQry .= 'networkmonthlycost, prevatinstallcost, installcost FROM fibrepackages ';
	...
	while($selData = mysqli_fetch_array($selRes))
	{
		...
		$retArr[$selData['packageid']]['networkinstallcost'] = $selData['networkinstallcost'];
		$retArr[$selData['packageid']]['prevatontcost'] = $selData['prevatontcost'];
		$retArr[$selData['packageid']]['prevatconnectcost'] = $selData['prevatconnectcost'];
		$retArr[$selData['packageid']]['prevatmonthlycost'] = $selData['prevatmonthlycost'];
		$retArr[$selData['packageid']]['networkmonthlycost'] = $selData['networkmonthlycost'];
		$retArr[$selData['packageid']]['prevatinstallcost'] = $selData['prevatinstallcost'];
		$retArr[$selData['packageid']]['installcost'] = $selData['installcost'];
	}
	return $retArr;
}

function getUnitOrders($Status = -1)
{
	global $dbCon;
	...
	$selQry .= 'networkconnectcost, networkontcost, networkinstallcost, prevatontcost, prevatconnectcost, prevatmonthlycost, networkmonthlycost, prevatinstallcost ';
	...
	if($Status != -1)
		$selQry .= 'AND orderstatus = ' . $Status . ' ';
	$selQry .= 'ORDER BY customerunits.unitnumber * 1';
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
		...
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['orderid'] = $selData['orderid'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['unitid'] = $selData['unitid'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['customerid'] = $selData['customerid'];
		...
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['prevatontcost'] = $selData['prevatontcost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['prevatconnectcost'] = $selData['prevatconnectcost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['prevatmonthlycost'] = $selData['prevatmonthlycost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['networkmonthlycost'] = $selData['networkmonthlycost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['prevatinstallcost'] = $selData['prevatinstallcost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['networkconnectcost'] = $selData['networkconnectcost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['networkontcost'] = $selData['networkontcost'];
		$retArr[$selData['unitid']]['orders'][$selData['orderid']]['networkinstallcost'] = $selData['networkinstallcost'];
	}
	return $retArr;
}

function getComplexOrders()
{
	global $dbCon;

	$selQry = 'SELECT orderid, customerunits.complexid, unitorderdetails.unitid, unitorderdetails.customerid, orderdate, unitorderdetails.packageid, termnum, speedid, ontid, vendorid, ontcost, installcost, connectcost, monthlycost, prevatsalesprice, ';
	$selQry .= 'DATEDIFF(NOW(), orderdate) AS datediff, orderstatus, networkconnectcost, networkontcost, networkinstallcost, prevatontcost, prevatconnectcost, prevatmonthlycost, networkmonthlycost, prevatinstallcost ';
	$selQry .= 'FROM unitorderdetails ';
	$selQry .= 'INNER JOIN customerunits ON customerunits.unitid = unitorderdetails.unitid ';
	$selQry .= 'WHERE orderid > 0 AND historyserial = 0 AND customerunits.complexid > 0 AND unitorderdetails.unitid > 0 ';
	$selQry .= 'ORDER BY customerunits.unitnumber * 1';
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
		$retArr[$selData['complexid']][$selData['unitid']]['orderid'] = $selData['orderid'];
		$retArr[$selData['complexid']][$selData['unitid']]['unitid'] = $selData['unitid'];
		$retArr[$selData['complexid']][$selData['unitid']]['customerid'] = $selData['customerid'];
		// $retArr[$selData['unitid']]['orderdate'] = $selData['orderdate'];
		$retArr[$selData['complexid']][$selData['unitid']]['packageid'] = $selData['packageid'];
		$retArr[$selData['complexid']][$selData['unitid']]['termnum'] = $selData['termnum'];
		$retArr[$selData['complexid']][$selData['unitid']]['speedid'] = $selData['speedid'];
		$retArr[$selData['complexid']][$selData['unitid']]['ontid'] = $selData['ontid'];
		$retArr[$selData['complexid']][$selData['unitid']]['vendorid'] = $selData['vendorid'];
		$retArr[$selData['complexid']][$selData['unitid']]['ontcost'] = $selData['ontcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['installcost'] = $selData['installcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['connectcost'] = $selData['connectcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['prevatsalesprice'] = $selData['prevatsalesprice'];
		$retArr[$selData['complexid']][$selData['unitid']]['orderdate'] = $selData['orderdate'];
		$retArr[$selData['complexid']][$selData['unitid']]['datediff'] = $selData['datediff'];
		$retArr[$selData['complexid']][$selData['unitid']]['orderstatus'] = $selData['orderstatus'];
		$retArr[$selData['complexid']][$selData['unitid']]['prevatontcost'] = $selData['prevatontcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['prevatconnectcost'] = $selData['prevatconnectcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['prevatmonthlycost'] = $selData['prevatmonthlycost'];
		$retArr[$selData['complexid']][$selData['unitid']]['networkmonthlycost'] = $selData['networkmonthlycost'];
		$retArr[$selData['complexid']][$selData['unitid']]['prevatinstallcost'] = $selData['prevatinstallcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['networkconnectcost'] = $selData['networkconnectcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['networkontcost'] = $selData['networkontcost'];
		$retArr[$selData['complexid']][$selData['unitid']]['networkinstallcost'] = $selData['networkinstallcost'];
	}
	return $retArr;
}
...
function getComplexUnitPackages($ComplexID)
{
	...
	while($selData = mysqli_fetch_array($selRes))
	{
		...
		$retArr[$selData['unitid']][$selData['orderid']]['prevatontcost'] = $selData['prevatontcost'];
		$retArr[$selData['unitid']][$selData['orderid']]['prevatconnectcost'] = $selData['prevatconnectcost'];
		$retArr[$selData['unitid']][$selData['orderid']]['prevatmonthlycost'] = $selData['prevatmonthlycost'];
		$retArr[$selData['unitid']][$selData['orderid']]['networkmonthlycost'] = $selData['networkmonthlycost'];
		$retArr[$selData['unitid']][$selData['orderid']]['prevatinstallcost'] = $selData['prevatinstallcost'];
		$retArr[$selData['unitid']][$selData['orderid']]['networkconnectcost'] = $selData['networkconnectcost'];
		$retArr[$selData['unitid']][$selData['orderid']]['networkontcost'] = $selData['networkontcost'];
		$retArr[$selData['unitid']][$selData['orderid']]['networkinstallcost'] = $selData['networkinstallcost'];
	}
	return $retArr;
}
...
function getUnitPackage($UnitID)
{
	...
	$selQry = 'SELECT orderid, unitid, customerid, orderdate, packageid, termnum, speedid, ontid, vendorid, ontcost, installcost, connectcost, monthlycost, prevatsalesprice, orderstatus, ';
	$selQry .= 'networkconnectcost, networkontcost, networkinstallcost, prevatontcost, prevatconnectcost, prevatmonthlycost, networkmonthlycost, prevatinstallcost ';
	$selQry .= 'FROM unitorderdetails WHERE historyserial = 0 AND unitid = ' . $UnitID;
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
		...
		$retArr[$selData['orderid']]['prevatontcost'] = $selData['prevatontcost'];
		$retArr[$selData['orderid']]['prevatconnectcost'] = $selData['prevatconnectcost'];
		$retArr[$selData['orderid']]['prevatmonthlycost'] = $selData['prevatmonthlycost'];
		$retArr[$selData['orderid']]['networkmonthlycost'] = $selData['networkmonthlycost'];
		$retArr[$selData['orderid']]['prevatinstallcost'] = $selData['prevatinstallcost'];
		$retArr[$selData['orderid']]['networkconnectcost'] = $selData['networkconnectcost'];
		$retArr[$selData['orderid']]['networkontcost'] = $selData['networkontcost'];
		$retArr[$selData['orderid']]['networkinstallcost'] = $selData['networkinstallcost'];
	}
	return $retArr;
}
...
function getUnitOrderByID($OrderID)
{
	...
	$selQry = 'SELECT orderid, unitid, customerid, orderdate, packageid, termnum, speedid, ontid, vendorid, ontcost, installcost, connectcost, monthlycost, prevatsalesprice, orderstatus, ';
	$selQry .= 'networkconnectcost, networkontcost, networkinstallcost, prevatontcost, prevatconnectcost, prevatmonthlycost, networkmonthlycost, prevatinstallcost ';
	$selQry .= 'FROM unitorderdetails WHERE orderid = ' . $OrderID;
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	return $selData = mysqli_fetch_array($selRes);
	// $retArr = array();
	// while($selData = mysqli_fetch_array($selRes))
	// {
		...
		// $retArr['prevatontcost'] = $selData['prevatontcost'];
		// $retArr['prevatconnectcost'] = $selData['prevatconnectcost'];
		// $retArr['prevatmonthlycost'] = $selData['prevatmonthlycost'];
		// $retArr['networkmonthlycost'] = $selData['networkmonthlycost'];
		// $retArr['prevatinstallcost'] = $selData['prevatinstallcost'];
	// }
	// return $retArr;
}

function getUnitOrdersByIDList($OrderIDs)
{
	global $dbCon;
	
	$selQry = 'SELECT orderid, unitid, customerid, orderdate, packageid, termnum, speedid, ontid, vendorid, ontcost, installcost, connectcost, monthlycost, prevatsalesprice, orderstatus, ';
	$selQry .= 'networkconnectcost, networkontcost, networkinstallcost, prevatontcost, prevatconnectcost, prevatmonthlycost, networkmonthlycost, prevatinstallcost ';
	$selQry .= 'FROM unitorderdetails WHERE orderid IN (' . $OrderIDs . ')';
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
...
function getUnitOrderStatusses()
{
	global $dbCon;

	$selQry = 'SELECT statusid, statusname FROM unitorderstatusses';
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	$retArr = array();
	while($selData = mysqli_fetch_array($selRes))
	{
		$retArr[$selData['statusid']] = $selData['statusname'];
	}
	return $retArr;
}
...
function addPasswordReset($SaveArr)
{
	global $dbCon;
	...
	$fieldA[$cnt] = 'requested';
	$valueA[$cnt] = 'NOW()';
	$cnt++;
	$updQry = 'INSERT INTO passwordresets(' . implode(",", $fieldA) . ') VALUES (' . implode(",", $valueA) . ')';
	$updRes = mysqli_query($dbCon, $updQry) or logDBError(mysqli_error($dbCon), $updQry, __FILE__, __FUNCTION__, __LINE__);
	return mysqli_insert_id($dbCon);
}

function getPasswordReset($ResetID)
{
	global $dbCon;
	
	$selQry = 'SELECT resetid, customerid, requested, randkey, timekey FROM passwordresets WHERE resetid = ' . $ResetID;
	$selRes = mysqli_query($dbCon, $selQry) or logDBError(mysqli_error($dbCon), $selQry, __FILE__, __FUNCTION__, __LINE__);
	return mysqli_fetch_array($selRes);
}
?>